...
- Verified local code HEAD before this state-refresh commit: `3d8c2e851d91804c2fbfe1ee45e83d1ffb9a5227` (`3d8c2e8`)
...
Strict mypy slices now cover `src.quality.validators.*`, `src.serving.api.auth.*`, `src.quality.monitors.*`, `src.serving.semantic_layer.*`, `src.serving.backends.*`, `src.orchestration.dags.*` (HEAD `80316fb`; surfaced + fixed the DuckDB `fetchone()` None-indexing bug), `src.processing.event_replayer` (HEAD `8a50ab6`), `src.processing.local_pipeline` (HEAD `98a9ed5`), and `src.processing.outbox` (HEAD `0953fcc`; `_connection` property guards use-after-close). All of `src/processing` except the PR-#23-gated `flink_jobs` is now strict-typed, plus eight `src/serving/api` slices: `src.serving.api.middleware.*` (HEAD `4ad01fd`), `src.serving.api.routers.deadletter` (HEAD `e92a6eb`), `src.serving.api.routers.webhooks` (code HEAD `45d3fc5`), `src.serving.api.routers.alerts` (code HEAD `5f61fd3`), `src.serving.api.routers.contracts` (code HEAD `84c63dc`), `src.serving.api.routers.agent_query` (code HEAD `0cdac06`), `src.serving.api.routers.batch` (code HEAD `0729fe5`), and `src.serving.api.routers.search` (code HEAD `3d8c2e8`). Measured after the search slice, the remaining untyped-def baseline is 52 mypy errors across 48 functions: `src/serving/api` has 37 errors across 36 functions in 12 files (largest clusters: `routers/admin.py`=12, `main.py`=6, `routers/admin_ui.py`=4) plus `src/processing/flink_jobs` has 15 errors across 12 functions in 3 files (gated by PR #23 / Docker). This supersedes the older 43-count advisory baseline, which undercounted some helper modules and mixed error/function units. The `src/serving/api` remainder is incremental, not load-bearing; pick it up only as bounded, individually-verified slices (one coherent module/package at a time, typing-policy assertion + full `mypy src` + broad unit + six green workflows each).
