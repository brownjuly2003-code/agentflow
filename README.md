...
- **Measured event-to-metric freshness**: an event entering the pipeline is reflected in `GET /v1/metrics/*` in **1.08 s p50 / 1.97 s p95** on production defaults (event-driven cache invalidation), tunable to **210 ms p50**; a plain TTL cache on the same pipeline sits at ~15 s p50. Reproducible: `python scripts/benchmark_freshness.py` → [docs/freshness-benchmark.md](docs/freshness-benchmark.md)
