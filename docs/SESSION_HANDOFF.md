...
**Last updated:** 2026-05-25 (session 24 — Kimi audit Low-priority cleanup: L-C1 obfuscation rename + L-C2 MySQL server.id env override + L-C3 redundant `==` drops)
**HEAD:** `356715e` on `main` locally (sessions 22+23); session 24 commit pending (touches `src/ingestion/connectors/{mysql,postgres}_cdc.py`, `src/quality/validators/{schema,semantic}_validator.py`, `tests/unit/test_cdc_connector_configs.py`, `CHANGELOG.md`, `docs/SESSION_HANDOFF.md`)
