...
**Last updated:** 2026-05-25 (session 22 — Kimi audit code-level immediate closure: H-C3 audit publish moved out of DB retry loop, M-C1 search rebuild wrapped for graceful degradation, R8 stale coverage.xml deleted locally)
**HEAD:** `4df9714` on `main` for session 21; session 22 commit pending (substantive bundle for session 22 is one commit touching `src/serving/api/auth/middleware.py`, `src/serving/api/main.py`, `tests/unit/test_audit_publisher.py`, `tests/unit/test_lifespan_search_resilience.py`, `CHANGELOG.md`, `docs/SESSION_HANDOFF.md`)
