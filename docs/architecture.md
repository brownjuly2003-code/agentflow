...
AgentFlow is an event-native metrics layer: business metrics are generated by operational events and stay live — the serving cache is invalidated when events arrive, with a measured **3.02 s p50 / 5.70 s p95** event-to-metric delay on the real Kafka→Flink→bridge path ([S8 e2e benchmark](perf/freshness-e2e-realpath.md)) and 1.06 s p50 / 1.99 s p95 on the in-process demo shortcut, which skips Kafka/Flink ([2026-06-06 demo snapshot](archive/performance/freshness-benchmark-2026-06-06.md)). Each metric declares which events move it through versioned contracts, so the event→metric graph is a tested artifact rather than tribal knowledge.
