...
- ✅ **dbt docs screencast** (session 8, 2026-05-24) — `docs/dv2-multi-branch/demo_dbt_docs.mp4` (~55 s, 1.7 MB). Playwright проходит overview → customer_360 (columns + description) → branch_pnl (с lineage graph showing rv.bv_order_canonical → branch_pnl) → returns_velocity (с lineage). Same TTS pipeline. Reproducible через `demo_dbt_docs.capture.py` + `demo_dbt_docs.narration.txt` + новый `infrastructure/dv2/dbt/dbt-docs-pod.yaml` (Pod вместо Job для `dbt docs serve --port 8080`).
...
5. Открытые задачи (deferred, нужен явный ask): **Phase 3 ops work** — A04 Debezium production CDC ждёт user-side decisions (hosts/creds/network/owners; cdc-production-onboarding.md runbook готов); A05 prod cluster validation требует prod access; A03 next p99 iter gated на CI runner upgrade decision (local 167ms достигнут, CI 800-1000ms — hardware, не код). Все DV2 screencasts (terminal cast + Argo+MinIO + dbt docs) сняты в session 6-8.
