...
- Открытое (deferred — needs explicit user ask):
  ...
  - **Satellite seeding для остальных филиалов** (spb/ekb/dxb/ala order header+pricing; spb/ekb/ala customer PII satellites) — нужно сначала DDL новых satellites, потом seed
