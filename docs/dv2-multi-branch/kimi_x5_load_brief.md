...
```bash
kaggle datasets download -d shonenkov/x5retailheroupliftrawdata \
    -p /path/to/x5 --unzip
```
...
SELECT splitByChar('-', store_code)[1] AS branch,
       count() AS stores
FROM rv.hub_store
GROUP BY branch
...
* warehouse/agentflow/dv2/loaders/x5_retail_hero/ executed against
  Kaggle dataset shonenkov/x5retailheroupliftrawdata, load_ts
