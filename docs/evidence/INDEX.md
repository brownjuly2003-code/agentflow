...
| [entity-benchmark-contract.md](../perf/entity-benchmark-contract.md) | Current benchmark reference | `scripts/profile_entity.py` writes ignored `.artifacts/perf-smoke/entity-profile.json` and refuses output under `docs/perf/`; `scripts/run_benchmark.py` owns the full-load family. | Measurement procedure, not a measured result and not an evidence identity. |
...
Kafka-to-live-metric claim, while the archived generated demo snapshot remains
the shortcut baseline and explicitly records that its pre-S7 invalidation
