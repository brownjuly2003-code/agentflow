...
### Как в AgentFlow
Смешанный профиль нагрузки живёт в [../tests/load/locustfile.py](../tests/load/locustfile.py): entity lookups, metric queries, NL query, batch и health. Сравнение baseline/current выполняет [../scripts/check_performance.py](../scripts/check_performance.py) в CI/load gates, а PR-job [../.github/workflows/perf-regression.yml](../.github/workflows/perf-regression.yml) теперь держит быстрый entity-only `perf-smoke` с порогом `p99 <= 500 ms`.
