...
write this lifecycle page, an archived snapshot, or the canonical
`docs/benchmark-baseline.json`, so a benchmark run cannot silently mutate
tracked documentation or its release-gate input.
...
`docs/benchmark-baseline.json` through `scripts/check_performance.py`. The
baseline is a reviewed, read-only gate input calibrated by the dated
[2026-05-24 CI hardware decision](ci-hardware-gap-2026-05-24.md), not a runtime
output generated by the benchmark harness. This is the only current full-load
comparison path.
