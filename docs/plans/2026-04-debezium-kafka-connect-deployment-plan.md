...
| Kafka Connect Helm chart | Done: `helm/kafka-connect/` contains worker deployment, service, secrets, values schema, connector hooks, topic bootstrap hook, NetworkPolicy + PDB + pod/container securityContext + `/tmp` memory emptyDir |
...
| Chart live validation in CI | Done: `tests/integration/test_helm_values_live_validation.py` is parametrized across `helm/agentflow` + `helm/kafka-connect` against a kind cluster; supports external context via `AGENTFLOW_LIVE_REUSE_CLUSTER=1` |
