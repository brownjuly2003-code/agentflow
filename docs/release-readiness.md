...
| CDC production onboarding | Not applicable under the 2026-06-05 operator decision: no production deployment or source exists in the current plan; enablement is not claimed; archived decision record and no-go gates remain in [Production CDC Source Onboarding](operations/cdc-production-onboarding.md) |
...
- Public benchmark on production hardware is closed as not applicable under the 2026-06-05 operator decision (no cloud budget or payment card, same constraint as the AWS closure): the checked-in single-node baseline is the only and sufficient release evidence, and a production-hardware result is not claimed. Archived plan: [Public Production-Hardware Benchmark Plan](perf/public-production-hardware-benchmark-plan.md). Reopen only on approved hardware access and budget.
- Chaos full suite runs on schedule; PR path covers smoke scope only.
- Production CDC source onboarding is closed as not applicable under the 2026-06-05 operator decision: no production deployment or production Postgres/MySQL source exists in the current plan, so the missing owner/decision record is expected and must not be counted as a release weakness. The checked-in local/demo and Kubernetes-shaped CDC path remains the project evidence; production CDC enablement is not claimed. Archived approval checklist: [Production CDC Source Onboarding](operations/cdc-production-onboarding.md). Reopen only when a real production source with named owners is provided.
- External pen-test attestation is closed as not applicable under the 2026-06-05 operator decision: there is no budget for a third-party test and no enterprise customer requiring one, so its absence is expected and must not be counted as a release weakness. Internal security audit evidence remains separate from third-party attestation, which is not claimed. Archived handoff: [External Pen-Test Attestation Handoff](operations/external-pen-test-attestation-handoff.md). Reopen only when a real report/attestation is provided.
...
| Production CDC source onboarding | N/A | Closed as not applicable on 2026-06-05 by operator decision: no production deployment or production source exists in the current plan. Production CDC enablement is not claimed. Earlier 2026-05-04 access triage (no owners, scope, network path, or production context) remains the recorded evidence trail. Archived handoff: `docs/operations/cdc-production-onboarding.md`. |
...
| Public production-hardware benchmark | N/A | Closed as not applicable on 2026-06-05 by operator decision: no cloud budget or payment card (same constraint as the AWS closure). The checked-in single-node baseline remains the only claimed evidence. Earlier 2026-05-04 access triage remains the recorded evidence trail. Archived plan: `docs/perf/public-production-hardware-benchmark-plan.md`. |
