...
AgentFlow закрыл технические блокеры из BCG audit от 2026-04-12 и довёл функциональную часть v1.0.0 до рабочего состояния. Поверх v13.5 security refresh v15 закрыл и GTM/documentation хвост: `BCG_audit.md` восстановлен, narrative API reference, competitive analysis, security audit, landing page, and Fly.io demo config are now part of the release evidence. `bandit_diff.py` остаётся зелёным against the checked-in baseline, а полный verification стек для closeout выполнен с одним локальным нюансом в chaos-команде из плана.
...
|-------|--------|----------|
| `docker compose up -d redis` + `python -m pytest tests/unit tests/integration tests/sdk --tb=line -q` | ✅ PASS | 543 passed in 224.12s after Redis became healthy; the first attempt hit a startup race in `test_analytics_logging_is_non_blocking` |
...
| `docker compose -f docker-compose.chaos.yml up -d` + `python -m pytest tests/chaos/test_chaos_smoke.py -v --timeout=180` + `docker compose ... down` | ⚠️ Documented failure | exact sequence conflicts with `tests/chaos` fixture on port `8474`; after cleanup, direct `python -m pytest tests/chaos/test_chaos_smoke.py -v --timeout=180` passed `3 passed in 49.03s` |
...
AgentFlow is technically release-ready. Code-level gates are green, v15 GTM/documentation assets are now part of the release evidence, and remaining open items are manual environment setup (`staging`/`prod` reviewers, AWS OIDC role), public benchmark publication on production hardware, external security attestation, and post-release PMF work.
