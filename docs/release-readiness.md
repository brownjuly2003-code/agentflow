...
| CDC production onboarding | In progress (2026-06-05 reopen): real operator-owned Neon source, solo-org decision record filled, capture channel shipped; waiting on the operator enabling Logical Replication. Decision record: [Production CDC Source Onboarding](operations/cdc-production-onboarding.md) |
...
- Phase 1 PMF work is closed as not applicable under the 2026-06-05 operator decision: AgentFlow has no active commercial sales motion, so missing replies, interviews, PMF scoring, pricing/WTP evidence, and paying-customer signals are expected and must not be counted as a release weakness. PMF validation is still not claimed. Archived handoffs: [Customer Discovery Tracker](customer-discovery-tracker.md) and [Pricing Validation Plan](pricing-validation-plan.md). Reopen only on real customer evidence or an operator-restarted sales motion.
...
- Real Terraform `apply` has not been executed from GitHub Actions and is no longer an active gate for the current plan. A managed-AWS production deployment is out of scope for this pre-production portfolio project (a deliberate non-goal); reopen only if the operator explicitly reintroduces an AWS account and approval.
...
- AWS OIDC role setup for GitHub Actions is archived as an optional path only. A managed-AWS production deployment is a deliberate non-goal for this pre-production portfolio project — the stack is validated end-to-end on a local/kind demo instead — so missing `AWS_TERRAFORM_ROLE_ARN`, tfvars, CloudTrail, or apply evidence should not be counted as a release weakness. Current archived handoff: [AWS OIDC Setup For Terraform Apply](operations/aws-oidc-setup.md).
...
- Public benchmark on ARM server hardware is DONE under the 2026-06-05 operator-amended class: a real run on the free GitHub-hosted arm64 runner (`ubuntu-24.04-arm`, Neoverse-N2, 4 vCPU) passed every release gate with zero failures — see [ARM Server Benchmark 2026-06-05](perf/arm-server-benchmark-2026-06-05.md). No `c8g.4xlarge` claim is made (4 vCPU shared runner, honestly recorded); the original `c8g.4xlarge+` plan remains the preferred path if production-grade hardware becomes available.
- Chaos full suite runs on schedule; PR path covers smoke scope only.
- Production CDC source onboarding is in progress under the second 2026-06-05 operator decision: the real production source is the operator-owned Neon Postgres backing VacancyRadar, the solo-org decision record is filled, and the dispatch-only capture workflow with unconditional teardown is shipped. The one remaining step is the operator enabling Logical Replication on the Neon project (irreversible). Until the capture run lands, production CDC capture is not claimed. Checklist: [Production CDC Source Onboarding](operations/cdc-production-onboarding.md).
- External pen-test attestation is closed as not applicable under the 2026-06-05 operator decision: a third-party test is out of scope for this pre-production portfolio project and no enterprise customer requires one, so its absence is expected and must not be counted as a release weakness. Internal security audit evidence remains separate from third-party attestation, which is not claimed. Archived handoff: [External Pen-Test Attestation Handoff](operations/external-pen-test-attestation-handoff.md). Reopen only when a real report/attestation is provided. Separately, a $0 automated posture channel was added on 2026-06-05 — OpenSSF Scorecard plus a prepared Best Practices self-assessment ([OpenSSF Security Posture](operations/openssf-security-posture.md)) — which are posture signals, not the third-party attestation, and do not change this N/A status.
...
## External Gate Criticality

These gates do not block local development, package publication, demos, or
continued application work. They do block stronger production, enterprise, and
compliance claims:

| Gate | Practical impact while open | Claim blocked |
| --- | --- | --- |
| AWS OIDC Terraform apply | N/A for the current plan: a managed-AWS production deployment is a deliberate non-goal for this pre-production portfolio project. Reopen only on explicit operator request. | "AWS infrastructure is managed by approved GitHub OIDC Terraform apply." |
| External pen-test attestation | N/A for the current plan under the 2026-06-05 operator decision: no third-party-test budget and no enterprise customer requiring one. Reopen only on a real report/attestation. | "A third party completed and retested a penetration test." |
...
- [x] Production CDC source onboarding removed from active release gates under the 2026-06-05 operator decision (no production deployment or source exists in the current plan; enablement is not claimed; archived checklist in `docs/operations/cdc-production-onboarding.md`)
...
- [x] Phase 1 PMF work removed from active release gates under the 2026-06-05 operator decision (no active commercial sales motion; tracker content remains synthetic/modelled and explicitly non-evidence; PMF validation is not claimed; archived handoffs in `docs/customer-discovery-tracker.md` and `docs/pricing-validation-plan.md`)
...
| `.venv\Scripts\python.exe -m pytest -p no:schemathesis -q --basetemp .tmp\npm-revocation-docs-basetemp -o cache_dir=.tmp\npm-revocation-docs-cache` with project-local `TMP`/`TEMP` | ✅ PASS | 752 passed, 4 skipped in 488.69s on 2026-05-04 after clarifying that legacy `NPM_TOKEN` revocation still needs a new-package trusted-publish run and accepted intake evidence |
| `.venv\Scripts\python.exe -m pytest -p no:schemathesis -q --basetemp .tmp\pi-skill-intake-note-basetemp -o cache_dir=.tmp\pi-skill-intake-note-cache` with project-local `TMP`/`TEMP` | ✅ PASS | 752 passed, 4 skipped in 476.13s on 2026-05-04 after adding the project-local Pi skill note to the intake checklist |
| `.venv\Scripts\python.exe -m pytest -p no:schemathesis -q --basetemp .tmp\release-evidence-final-sync-basetemp -o cache_dir=.tmp\release-evidence-final-sync-cache` with project-local `TMP`/`TEMP` | ✅ PASS | 752 passed, 4 skipped in 398.35s on 2026-05-04 after final release-evidence sync |
...
| Docker API image README-copy regression on `67c2ae5` | ✅ PASS | `Dockerfile.api` and `scripts/k8s_staging_up.sh` copy `README.md` before package installs; `docker-compose.prod.yml` now reuses `Dockerfile.api` instead of carrying a separate inline API image recipe. GitHub checks for `67c2ae5` were green: `lint`, `test-unit`, `test-integration`, `perf-check`, `helm-schema-live`, `schema-check`, `terraform-validate`, `bandit`, `safety`, `npm-audit`, `trivy`, `e2e`, `load-test`, and `staging` |
...
| AWS OIDC Terraform apply readiness | N/A | A managed-AWS production deployment is a deliberate non-goal for this pre-production portfolio project (decision recorded 2026-05-30). Keep `.github/workflows/terraform-apply.yml` disabled and preserve [AWS OIDC Setup](operations/aws-oidc-setup.md) only as an archived optional path. Do not treat missing AWS role, tfvars, CloudTrail, or apply evidence as a project deficiency unless AWS is explicitly reintroduced with an account and approval. |
...
| Production CDC source onboarding | ⏳ IN PROGRESS | Reopened 2026-06-05: a real production source exists (operator-owned Neon Postgres backing VacancyRadar, `public.vacancies` ~95k rows). Solo-org decision record filled; dispatch-only capture workflow + secrets + teardown-trapped script shipped. Waiting on one operator action: enable Logical Replication on the Neon project (irreversible `wal_level` flip). Handoff: `docs/operations/cdc-production-onboarding.md`. |
| Phase 1 PMF and pricing evidence | N/A | Closed as not applicable on 2026-06-05 by operator decision: no active commercial sales motion. PMF/pricing validation is not claimed; tracker content remains synthetic/modelled and non-evidence. Earlier 2026-05-04 access triage remains the recorded evidence trail. Archived handoffs: `docs/customer-discovery-tracker.md` and `docs/pricing-validation-plan.md`. |
| Public production-hardware benchmark | ✅ PASS (amended class) | Real ARM-server run on 2026-06-05: `ubuntu-24.04-arm` free runner (Neoverse-N2, 4 vCPU), canonical harness, 554 requests / 0 failures, aggregate p50 6 ms / p99 150 ms, all entity gates passed. Evidence: `docs/perf/arm-server-benchmark-2026-06-05.md` + run 27012731848. No `c8g.4xlarge` claim; original plan stays preferred if production-grade hardware becomes available. |
| External pen-test attestation | N/A | Closed as not applicable on 2026-06-05 by operator decision: no third-party-test budget and no enterprise customer requiring one. Third-party attestation is not claimed; internal audit evidence stays distinct. Earlier 2026-05-06 access triage remains the recorded evidence trail. Archived handoff: `docs/operations/external-pen-test-attestation-handoff.md`. |
...
- After one successful trusted-publish workflow run for
  `@yuliaedomskikh/agentflow-client`, revoke the old `NPM_TOKEN`. Do not use
  the green `2c72387` publish run as revocation proof; it published the legacy
  npm package with `NODE_AUTH_TOKEN`.

## Release Verdict

**v1.1.0 release line prepared; post-release local gates recorded.**

AgentFlow is publicly available and the current checked-in docs/code describe the intended release, npm Trusted Publishing, Docker-backed verification, and CDC state. AWS Terraform apply is intentionally out of scope under the operator's no-budget/no-card constraint (2026-05-30); on 2026-06-05 the operator extended the same decision to the remaining four external gates — production CDC source onboarding, Phase 1 PMF/pricing evidence, the production-hardware benchmark, and external pen-test attestation. All five are closed as not applicable for the current plan: their absence must not be listed as an active release blocker or project weakness, and the claims they would unlock (production CDC enabled, validated PMF/pricing, production-hardware results, third-party attestation) remain explicitly NOT made. Reopen any of them only when the operator provides the corresponding real external inputs; route new evidence through the [External Gate Evidence Intake Checklist](operations/external-gate-evidence-intake.md). Remaining open items:
