...
| `docker compose up -d redis` + project-local `TEMP`/`TMP` + `python -m pytest -p no:schemathesis -q --tb=short --durations=30 --timeout=300 --basetemp D:\DE_project\.tmp\pytest-basetemp-doc-post-merge-gate` | ✅ PASS | 668 passed, 8 skipped, 13 warnings in 496.93s on 2026-04-27 |
