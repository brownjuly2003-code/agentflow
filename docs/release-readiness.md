...
| Recorded full-suite evidence | Green locally on 2026-04-27 on top of `8d7088d`: 668 passed, 8 skipped, 12 warnings in 617.13s with Redis running and project-local pytest temp paths. |
...
| `docker compose up -d redis` + project-local `TEMP`/`TMP` + `python -m pytest -p no:schemathesis -q --tb=short --durations=30 --timeout=300 --basetemp=.tmp\pytest-doc-sync -o cache_dir=.tmp\.pytest-cache-doc-sync` | ✅ PASS | 668 passed, 8 skipped, 12 warnings in 617.13s on 2026-04-27 |
