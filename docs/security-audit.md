...
**Updated:** 2026-09-03 (production Ingress rules cannot route /metrics; NetworkPolicy must enumerate the scrape namespace)
...
7. `/metrics` remains unauthenticated to anything that can already reach the pod port; no monitoring identity (mTLS, auth proxy, or IP allowlist) is implemented. Production Ingress rules that would send `/metrics` to the API are refused at `helm template` time, but that values-contract check does not authenticate the endpoint, does not bind the chart's dev/demo defaults, and does not cover ingress-controller annotations that rewrite a contract-compliant path onto `/metrics` (recorded as a follow-up). The contract also leaves `service.type` unchecked, so `NodePort`/`LoadBalancer` publishes the port directly, and `ingress.enabled=false` moves routing outside the chart (recorded as a follow-up). Production NetworkPolicy must name a scrape namespace in `networkPolicy.ingressFromNamespaces` (`values-production.yaml` ships no guessed namespace; the unedited overlay is refused). An empty list renders `ingress: []` (deny all) and is refused because neither the ingress controller nor Prometheus can reach the service port. A list whose every `kubernetes.io/metadata.name` is `ingress-nginx` is refused unless `networkPolicy.scrapeFromIngressNamespace=true` records that Prometheus shares the ingress-controller namespace.
