...
4. **Strict-typing cadence** (incremental, not load-bearing) — promote one more
   ...
   and `src.serving.api.rate_limiter`.
   Remaining measured candidates after the rate-limiter slice: 35 untyped
   functions across 11 files in `src/serving/api` (`routers/admin.py`=12,
   `main.py`=6, `routers/admin_ui.py`=4, `alerts/dispatcher.py`=3,
   `webhook_dispatcher.py`=2, `routers/slo.py`=2, `routers/stream.py`=2,
   and `analytics.py` / `routers/lineage.py` / `security.py` /
   `versioning.py`=1 each) plus 15 errors across 12 functions in
