...
4. **Strict-typing cadence** (incremental, not load-bearing) — promote one more
   ...
   Remaining measured candidates after the admin UI slice: 21 untyped
   functions across 4 files in `src/serving/api` (`routers/admin.py`=12,
   `main.py`=6, `alerts/dispatcher.py`=2, and `webhook_dispatcher.py`=1).
   These four API-side files all require a focused Claude second opinion before
