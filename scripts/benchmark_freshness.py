"""Measure demo event-to-metric freshness and write ignored runtime artifacts.
...
The API process serves metrics through a Redis-backed query cache. This harness
varies the legacy webhook-dispatcher poll that invalidated metric keys before
S7. Current production wiring uses serving-bridge push plus an independent
``MetricCacheController`` journal scan; this benchmark does not measure that
real path.

- ``event_driven``   pre-S7 demo defaults: cache TTL 30 s, dispatcher poll 2 s.
- ``fast_poll``      same legacy wiring, with the poll interval tuned to 0.25 s.
...
REPORT_PATH = PROJECT_ROOT / ".artifacts" / "freshness" / "freshness-benchmark.md"
RESULTS_PATH = PROJECT_ROOT / ".artifacts" / "freshness" / "current.json"
CURRENT_LIFECYCLE_PATH = PROJECT_ROOT / "docs" / "perf" / "freshness-benchmark.md"
ARCHIVED_SNAPSHOT_PATH = (
    PROJECT_ROOT / "docs" / "archive" / "performance" / "freshness-benchmark-2026-06-06.md"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Measure demo event-to-metric freshness and write ignored runtime artifacts.",
    )
...
def resolve_report_path(report_path: str) -> Path:
    ...
    if resolved.resolve() in protected_paths:
        raise ValueError(
            "Tracked freshness documentation cannot be overwritten; write runtime reports "
            "under .artifacts/freshness/ instead."
        )
    return resolved
...
def build_report(
    *,
    ...
    lines = [
        ...
        "",
        "> Runtime artifact generated by `python scripts/benchmark_freshness.py`.",
        "> Re-running replaces `.artifacts/freshness/freshness-benchmark.md`.",
        "> Machine-readable results: `.artifacts/freshness/current.json`.",
        "> Scope: in-process DuckDB diagnostic of the pre-S7 demo dispatcher-poll wiring.",
        "> It does not represent current production invalidation wiring; use the S8",
        "> real-path record for Kafka -> Flink -> bridge -> ClickHouse -> API evidence.",
        "",
        ...
        "excluded). This arm isolates the pre-S7 webhook-dispatcher poll wiring.",
        "Current production invalidation is push-driven with an independent",
        "`MetricCacheController` journal scan.",
        "",
        "## System Under Test",
        "",
        f"- OS: `{system_info['os']}`",
    ...
    arm_rows = [
        (
            ...
            f"event-driven, {PRODUCTION_POLL_SECONDS:.0f} s poll (pre-S7 demo)",
        ),
        (
            "fast_poll",
            f"TTL {PRODUCTION_TTL_SECONDS} s",
    ...
    lines += [
        ...
        "",
        (
            "- `event_driven` isolates the pre-S7 demo configuration: staleness is bounded "
    ...
    lines += [
        (
            "- `fast_poll` shows the same pre-S7 demo wiring with the dispatcher poll "
            f"tuned to {FAST_POLL_SECONDS:.2f} s; it is not current production topology."
        ),
        (
        ...
        (
            "- In the pre-S7 demo wiring, the dispatcher's invalidation scan covered "
            "every new pipeline event "
        ...
        "",
        "Results vary by host, timestamp, and runtime state. Promote evidence only",
        "under a date-stamped filename with its JSON companion and exact",
        "run/host/source provenance.",
    ]
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agentflow-freshness-") as temp_dir:
        ...
        try:
            ...
            # it is already in; let one pre-S7 demo-length tick drain first.
