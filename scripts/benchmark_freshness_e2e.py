...
DEFAULT_REPORT = PROJECT_ROOT / ".artifacts" / "freshness" / "e2e-realpath.md"
CURRENT_LIFECYCLE_PATH = PROJECT_ROOT / "docs" / "perf" / "freshness-e2e-realpath.md"
ARCHIVED_SNAPSHOT_PATH = (
    PROJECT_ROOT / "docs" / "archive" / "performance" / "freshness-e2e-realpath-2026-07-09.md"
)
PROTECTED_OUTPUT_PATHS = (CURRENT_LIFECYCLE_PATH, ARCHIVED_SNAPSHOT_PATH)
...
def resolve_output_path(output_path: str) -> Path:
    ...
    protected_paths = {path.resolve() for path in PROTECTED_OUTPUT_PATHS}
    if resolved.resolve() in protected_paths:
        raise ValueError(
            "Tracked S8 freshness documentation cannot be overwritten; write runtime "
...
def read_metric(api_base: str, metric: str, window: str, api_key: str) -> float:
    ...
    try:
        ...
        if response.status != 200:
            raise RuntimeError(f"GET {path} -> {response.status}: {body[:300]!r}")
...
def build_markdown(report: dict) -> str:
    ...
    lines = [
        ...
        "",
        f"> Runtime artifact generated by `scripts/benchmark_freshness_e2e.py`. Measured: `{gen}`.",
        "> Re-running replaces `.artifacts/freshness/e2e-realpath.md`.",
        "> Machine-readable: `.artifacts/freshness/e2e-realpath-current.json`.",
        "> Results depend on host, time, and runtime state; this single-node diagnostic",
        "> is not a production SLA or acceptance record.",
        "> Promote evidence only under a date-stamped filename with exact source, host,",
        "> runtime, command, and JSON-companion provenance.",
        "",
        "## What is measured",
        "",
        ...
        (
            f"- Poll interval: {report['poll_interval_ms']} ms · "
        ...
        "",
        (
            "| Metric | Real path (this run) | In-process DuckDB shortcut* | "
            "Streaming hop only** | Entity real-path (S6 live) |"
        ),
        (
            "|--------|---------------------:|----------------------------:|"
            "---------------------:|---------------------------:|"
        ...
        (
            f"| p50    | **{format_duration(s['p50_ms'])}** | 1.06 s | "
            "2.50 s | 3.26 s (entity, n≈1) |"
        ),
...
def main() -> int:
    args = parse_args()
    try:
        json_path = resolve_output_path(args.report_json)
        md_path = resolve_output_path(args.report_md)
    ...
    print(
        f"  min/max : {format_duration(summary['min_ms'])} / {format_duration(summary['max_ms'])}"
    )
