...
def build_markdown(report: dict) -> str:
    ...
    lines = [
        ...
        f"> Machine-readable: `.artifacts/throughput/realpath-current.json`.",
        "",
        "## What is measured",
        "",
        ...
        f"| **Bridge apply rate** (produce start → catch-up) | **{format_rate(s['bridge_apply_eps'])}** |",
        f"| Catch-up wall time | {s['catchup_wall_s']:.2f} s |",
        f"| Lag start → end / peak sampled | {s['lag_start']:.0f} → {s['lag_end']:.0f} / {s['lag_peak']:.0f} |",
        "",
    ]
    if lat.get("samples"):
        lines += [
...
def main() -> int:
    ...
    parser.add_argument("--api-base", default=os.getenv("AGENTFLOW_API_BASE", "http://127.0.0.1:8000"))
