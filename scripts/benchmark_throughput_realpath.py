...
DEFAULT_REPORT = PROJECT_ROOT / ".artifacts" / "throughput" / "realpath-current.md"
CURRENT_LIFECYCLE_PATH = PROJECT_ROOT / "docs" / "perf" / "throughput-realpath.md"
ARCHIVED_SNAPSHOT_PATH = (
    PROJECT_ROOT / "docs" / "archive" / "performance" / "throughput-realpath-2026-07-09.md"
)
...
def resolve_report_path(report_path: str) -> Path:
    ...
    if resolved.resolve() in protected_paths:
        raise ValueError(
            "Tracked throughput documentation cannot be overwritten; write runtime reports "
            "under .artifacts/throughput/ instead."
        )
    return resolved
...
def build_markdown(report: dict) -> str:
    ...
    lines = [
        ...
        "",
        (
            "> Runtime artifact generated by `scripts/benchmark_throughput_realpath.py`. "
            f"Measured: `{gen}`."
        ),
        "> Re-running replaces `.artifacts/throughput/realpath-current.md`.",
        "> Machine-readable: `.artifacts/throughput/realpath-current.json`.",
        "> Results are host- and time-dependent; promote evidence only under a",
        "> date-stamped filename with exact run, host, and source provenance.",
        "",
        "## What is measured",
        "",
