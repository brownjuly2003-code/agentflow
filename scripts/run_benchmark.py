...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=("Run the AgentFlow Locust benchmark and write ignored runtime artifacts."),
    )
...
def build_report(
    *,
    ...
    lines = [
        ...
        "",
        (
            "> This runtime artifact is generated by `python scripts/run_benchmark.py`; "
            "the default report path is `.artifacts/benchmark/benchmark.md`. For "
            "entity-specific optimization evidence, see "
    ...
    lines.extend(
        [
            ...
            *build_comparison_section(aggregate, claims, endpoint_rows, users),
            ...
            (
                "- Keep mutable reports under `.artifacts/benchmark/`; promote only "
                "date-stamped release evidence into `docs/perf/`."
            ),
        ]
    )
    return "\n".join(lines) + "\n"
