...
- `last_latest_version` and `last_deprecated` accessors on sync and async clients for the latest version response headers
...
### Changed
- Prepared the Python SDK in version lockstep with AgentFlow runtime `2.1.0` and the TypeScript SDK; this line is not published until its release tag is authorized.
