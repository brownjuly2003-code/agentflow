...
(``src/agentflow_runtime/processing/flink_jobs/stream_processor.py``) — and, until this module,
...
* ``SERVING_BACKEND=clickhouse`` (production): a standalone process,
  ``python -m agentflow_runtime.processing.bridge_consumer``. ClickHouse is out-of-process and
...
from agentflow_runtime.processing.bridge_metrics import (
    APPLY_BATCH_SIZE,
    APPLY_FAILURES,
    CONSUMER_LAG,
    EVENTS_APPLIED,
...
# (`src/agentflow_runtime/processing/local_pipeline.py`). Its if/elif chain has no `else`, so an
...
class ServingBridge:
    ...
    def run_once(self) -> BatchResult | None:
        ...
        if result.applied_event_ids:
            ...
            try:
                from agentflow_runtime.serving.cache_invalidation import publish_metrics_invalidate
