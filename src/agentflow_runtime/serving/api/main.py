...
from agentflow_runtime.logger import configure_logging
...
from agentflow_runtime.serving.api.alert_dispatcher import AlertDispatcher
...
from agentflow_runtime.serving.api.routers.alerts import router as alert_router
from agentflow_runtime.serving.api.routers.batch import router as batch_router
...
from agentflow_runtime.serving.api.routers.ops import router as ops_router
from agentflow_runtime.serving.api.routers.search import router as search_router
...
from agentflow_runtime.serving.node import resolve_node_config
from agentflow_runtime.serving.node.emitter import NodeEmitter
from agentflow_runtime.serving.node.ingest import router as node_ingest_router
from agentflow_runtime.serving.node.seed import seed_node_baseline
...
async def _lifespan_body(app: FastAPI) -> AsyncIterator[None]:
    ...
    if app.state.demo_seed_on_boot:
        ...
        # `python -m agentflow_runtime.serving.provision` (or the bridge writer), and readiness
    ...
    if _truthy(os.getenv("AGENTFLOW_SERVING_BRIDGE_ENABLED", "false")):
        if app.state.query_engine._backend_name != "duckdb":
            ...
            try:
                from agentflow_runtime.processing.bridge_consumer import start_in_process_bridge
        ...
        checks.append(
            {
                ...
                if provisioned
                    ...
                    "serving tables are missing — run "
                    "`python -m agentflow_runtime.serving.provision --schema`"
                ),
            }
        )
