...
from agentflow_runtime.serving.backends import (
    BackendExecutionError,
    BackendMissingTableError,
    ServingBackend,
)
from agentflow_runtime.serving.semantic_layer.sql_literals import quote_sql_literal
...
class ClickHouseBackend(ServingBackend):
    ...
    * Identifiers (table / column / schema names) come from the catalog
    ...
    def ensure_schema(self) -> None:
        ...
        (``python -m agentflow_runtime.serving.provision --migrate``), and ``_assert_tenant_key``
        ...
        # DuckDB embedded schema (ADR 0012 N4, `src/agentflow_runtime/processing/local_pipeline.py`)
        # — deferred, not an oversight. The three-node demo's node-ingest write
        # path (`src/agentflow_runtime/serving/node/ingest.py`) always applies through
    ...
    def assert_tenant_key(self) -> None:
        ...
        is loud: the operator runs ``python -m agentflow_runtime.serving.provision --migrate``.
        """
        ...
        raise BackendExecutionError(
            ...
            "Migrate with: python -m agentflow_runtime.serving.provision --migrate"
        )
