...
pydantic model: the port must not import ``agentflow_runtime.serving.api.alerts`` (that
...
def get_control_plane_store(app: FastAPI) -> ControlPlaneStore:
    ...
    if kind == "embedded":
        # Deferred: agentflow_runtime.serving.api.alerts.dispatcher and webhook_dispatcher
        ...
        from agentflow_runtime.serving.api.alerts.dispatcher import get_alert_config_path
        from agentflow_runtime.serving.api.webhook_dispatcher import get_webhook_config_path
