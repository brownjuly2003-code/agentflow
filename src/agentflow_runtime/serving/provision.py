...
bring-up.
    ...
    python -m agentflow_runtime.serving.provision --schema         # idempotent DDL
    python -m agentflow_runtime.serving.provision --seed           # demo rows, only if empty
    python -m agentflow_runtime.serving.provision --schema --seed  # full demo bring-up
    python -m agentflow_runtime.serving.provision --migrate        # rebuild pre-tenant-key tables
...
from agentflow_runtime.serving.backends import (
    ServingBackend,
    create_backend,
    load_serving_backend_config,
)
from agentflow_runtime.serving.backends.duckdb_backend import DuckDBBackend
from agentflow_runtime.serving.transport_policy import assert_secure_transport, resolve_profile
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m agentflow_runtime.serving.provision",
