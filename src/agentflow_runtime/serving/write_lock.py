...
(``src/agentflow_runtime/serving/db_pool.py``), so every in-process writer that issues
...
* the center node-ingest endpoint (``src/agentflow_runtime/serving/node/ingest.py``), and
* the in-process serving bridge on the DuckDB backend
  (``src/agentflow_runtime/processing/bridge_consumer.py``), which cannot be a separate process
