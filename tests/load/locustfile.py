...
class AgentUser(HttpUser):
    ...
    def entity_lookup(self):
        ...
        if roll < 0.4:
            ...
            self.client.get(f"/v1/entity/order/{oid}", name="/v1/entity/order/[id]")
        elif roll < 0.7:
            uid = random.choice(USERS)
            self.client.get(f"/v1/entity/user/{uid}", name="/v1/entity/user/[id]")
        else:
            pid = random.choice(PRODUCTS)
            self.client.get(
                f"/v1/entity/product/{pid}", name="/v1/entity/product/[id]"
            )
    ...
    def nl_query(self):
        ...
        self.client.post("/v1/query", json={"question": question})
