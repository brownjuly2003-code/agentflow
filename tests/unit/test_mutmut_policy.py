...
#     functions) AND the assert_no_pii_access deny-gate (rejects any query that
#     could read a PII column); the H-6 projection-position bypass lived here, so a
#     surviving mutant in its checks is a guard bypass.
#   - auth manager / key rotation: API-key issue / verify / rotation.
#   - pii_policy: PII redaction on the entity path (the query path is denied by the
#     sql_guard gate above, so PII is never read).
...
# sql_guard.py, pii_policy.py, rate_limiter.py, sql_builder.py, nl_queries.py,
