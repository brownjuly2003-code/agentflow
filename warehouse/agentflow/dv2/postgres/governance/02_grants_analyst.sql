/*
Purpose: Allow-list grants for dv2_analyst (cross-branch analytics, NO contact
         PII) — PostgreSQL port of ../../governance/02_grants_analyst.sql.
Layer:   Governance (ADR 0006 Phase 2 follow-up).
Policy:  FAIL-CLOSED. Every readable object is granted here explicitly; a new
         ...
         (tests/unit/test_dv2_postgres_governance_ddl.py enforces that every
         satellite table under postgres/satellites/ is either granted below or
         listed in the DENIED block). NB: PostgreSQL block comments NEST — a
         literal "slash-star" glob inside this header would swallow the file.
The engine (PostgreSQL ACLs), not the application, is the boundary: `SELECT *`,
whole-row refs (`SELECT t FROM view t`, `to_jsonb(t)`) and positional
column-rename lists (`AS t(a, b, ...)` — expressible on PostgreSQL, unlike
ClickHouse) all fail with `permission denied` because column privileges are
checked on resolved attributes, not on SQL text or output names.

Forbidden fail-open patterns (pinned by the unit test):
  - GRANT ... ON ALL TABLES IN SCHEMA rv — would expose the next PII table by
    default;
  - ALTER DEFAULT PRIVILEGES — same, for future objects.
...
ClickHouse objects with no PostgreSQL counterpart (nothing to grant here):
bv_order_canonical_mat (CH materialized copy) and the marts schema (dbt marts
are built on the ClickHouse stand only).
...
-- email, phone, birth_date) are NOT granted. PostgreSQL views execute with
-- their owner's rights by default (the DEFINER analog; security_invoker is
-- never set on these views), so these grants work without exposing the
-- underlying personal satellites.
